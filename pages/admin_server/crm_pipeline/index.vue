    ...
    <base-header class="pb-6">
      <div class="row align-items-center py-4">
        ...
        <div class="col-lg-6 col-5 text-right">
          <nuxt-link to="/admin_server/crm_pipeline/add" class="btn btn-neutral btn-sm">Tambah Data</nuxt-link>
        </div>
      </div>
    </base-header>
      ...
      <div>
        <card class="no-border-card" body-classes="px-0 pb-1" footer-classes="pb-2">
          <template slot="header">
            <h3 class="mb-0">Template Pipeline</h3>
            <p class="text-sm mb-0">Manajemen Template Pipeline</p>
          </template>
          <div>
            ...
            <el-table class="mt--3" :data="tableData" row-key="id" header-row-class-name="thead-light">
              ...
              <el-table-column min-width="200px" align="right" label="Actions">
                <div slot-scope="{$index, row}" class="d-flex">
                  ...
                  <nuxt-link :to="{name: 'admin_server-crm_pipeline-edit', params: {id: row.id_}}">
...
//   mixins: [clientPaginationMixin],
  ...
  filters: {
    createTime(val) {
      ...
      return this.$moment(val, "YYYY-MM-DD")();
    }
  },
  ...
  data() {
    return {
      pipelineData: [],
      propsToSearch: ["createTime", "creatorName", "title ", "note"],
      tableColumns: [
        {
          prop: "creatorName",
          label: "Nama",
      ...
      title: "",
      selectedRows: [],
      url: "main_b/pipeline/get_pipeline",
      paging: {
        size: 10,
        totalElements: 0,
        totalPages: 0,
  ...
  methods: {
    ...
    deleteRow(row) {
      this.$axios
        .$delete(`main_b/pipeline/pipeline/` + row.id_)
        .then((user) => {
          swal.fire({
            title: "Terhapus",
