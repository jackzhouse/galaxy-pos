          ...
          <card header-classes="bg-transparent">
            ...
            <time-line type="one-side">
              ...
              >
                ...
                <br />
                <div>
                  <template v-if="item.isInvoice == true">
                      ...
                      >
                        <div class="col-md-12">
                          <div class="form-group row">
                            ...
                            <div class="col-md-3">
                              <button type="button" class="btn btn-primary btn-mn" @click="firstFormSubmit(item.nomor, item, 'form')">Save</button>
                              <button type="button" class="btn btn-danger btn-mn" @click="item.invoiceId = ''">Clear</button>
                            </div>
                          </div>
                        </div>
                        <div class="col-md-12" v-if="item.invoiceId">
                          <detail-invoice :invoiceId="item.invoiceId"></detail-invoice>
                        </div>
                      </form>
                    </validation-observer>
                  </template>
                  <template v-else>
                    <div v-if="item.isMultiForm == true">
                      <button
                        type="button"
                      ...
                      <div class="mt-3">
                        <badge
                          v-for="(formDetail,
                          indexFormDetail) in item.formValue"
                          :key="indexFormDetail"
                          rounded
                          type="default"
                          size="lg"
        ...
        <div>
          <validation-observer ref="form">
            <form
              class="needs-validation"
              @submit.prevent="
                firstFormSubmit(itemModal.nomor, itemModal, 'multi')
              "
            >
              <div class="col-md-12">
...
import DetailInvoice from "@/pages/crm/crm_customer/detail_invoice";
...
export default {
  ...
  methods: {
    async getInvoiceByCustomer() {
      await this.$axios
        .$get(
          `main_b/invoice/getComboInvoice/${this.$route.query.id}`
        )
        .then((response) => {
          console.log(response);
          this.invoiceOptions = response;
        });
    },
    async getDatas() {
      await this.$axios
        .$get(
          `main_b/pelanggan/get_pipeline_pelanggan/${this.$route.query.id}/${this.$route.query.idProcess}`
        )
        .then((response) => {
          console.log(response);
    ...
    firstFormSubmit(noProcess, process, type) {
      ...
      if (statusBefore == "done") {
        this.$axios
          .$put(
            `/main_b/pelanggan/update_pipeline_to_pelanggan/${this.$route.query.id}/${this.$route.query.idProcess}/` +
              noProcess +
              `/${type}`,
            process
          )
